...
const newGroup = TryCatch(async (req, res, next) => {
    ...
    if (members.includes(req?.user?.id)) {
        return errorMessage(next, 'You are already included in members, please provide other two or more members', 400);
    }
    ...
    const transFormedChats = chats.map((chat) => {
        ...
        return {
            ...
            avatar: chat.groupChat
                ? chat.members
                    .slice(0, 3)
                    .map((member) => typeof member !== 'string' && member.avatar.url)
...
const addMembers = TryCatch(async (req, res, next) => {
    ...
    const uniqueNewMembers = allNewMembers
